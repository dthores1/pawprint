...
// Search box to grant site-management access. Only members WITHOUT access are
// offered (admins are always allowed and excluded by the caller).
...
export function ManageSitesModal({ isOpen, onClose }: Props) {
  const {
    orgMembers,
    memberPermissions,
    people,
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Manage Sites Access" className="max-w-xl">
      <div className="space-y-4">
        <p className="text-text-secondary text-sm">
          Choose which members can create and edit rescue sites. Everyone can
          view sites; admins and owners can always manage them.
        </p>

        <MemberSearchAdd members={grantableMembers} onAdd={grantSitePermission} />

        <div className="rounded-xl border border-border overflow-hidden">
          <div className="divide-y divide-border">
            {allowedMembers.length === 0 ?
            <div className="px-5 py-8 text-center text-sm text-text-secondary">
                No one has site-management access yet. Search above to grant it.
              </div> :
            ...
            allowedMembers.map((m) => {
              ...
              return (
                <div key={m.id} className="flex items-center gap-3 px-5 py-3">
                  ...
                  <button
                    type="button"
                    onClick={() => revokeSitePermission(m.id)}
