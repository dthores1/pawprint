-- 0034_sitting_expired_status.sql
-- Adds an `expired` terminal status for sitting_requests and the nightly
-- cron that flips an unclaimed request to expired once its end date passes.
-- Also widens archive_record so an expired request can be archived (it's
-- audit history, same as completed / canceled).
--
-- NOTE: this captures the function the user added by hand. The original
-- copy had `where status = 'unclaimed'` — the DB column actually stores
-- 'open' for unclaimed rows (the UI relabels it "Unclaimed"), so that
-- predicate never matched. This migration uses the correct value.
--
-- Re-running is safe: the CHECK is dropped-and-recreated, the cron job is
-- unscheduled before being scheduled, and the function uses CREATE OR
-- REPLACE.

-- ---------- Allow 'expired' in the status CHECK ----------
alter table sitting_requests
  drop constraint if exists sitting_requests_status_check;
alter table sitting_requests
  add constraint sitting_requests_status_check
  check (status in ('open','claimed','in_progress','completed','canceled','expired'));

-- ---------- Expire-on-schedule function ----------
create or replace function expire_sitting_requests()
returns void
language sql
as $$
  update sitting_requests
  set status = 'expired',
      updated_at = now()
  where status = 'open'
...
do $$
begin
  -- Unschedule any prior job with the same name. Swallows the
  -- "job not found" exception on first install.
  perform cron.unschedule('expire-sitting-requests');
exception when others then
  null;
end $$;

select cron.schedule(
  'expire-sitting-requests',
  '5 0 * * *',
  $$select expire_sitting_requests();$$
);

-- ---------- archive_record: allow 'expired' on sitting_requests ----------
-- Replaces the function from 0030 (Phase 7). Only the sitting_requests
-- blocker changes; all other rules are unchanged.
...
begin
  ...
  if p_table = 'supply_requests' then
    ...
    -- Added 'expired' alongside completed/canceled — they're all terminal.
    if v_status not in ('completed','canceled','expired') then
      raise exception
  ...
  -- Cross-table blockers for animals / people / litters (unchanged from 0030)
    ...
    if exists (
      ...
      where fp.animal_id = p_id
        ...
        and sit.status not in ('completed','canceled','expired')
    ) then
      raise exception
